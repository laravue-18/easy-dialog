    ...
    addNewBox(action, prev_row_id, val){
      ...
      if(this.insert == 0){
        ...
        if(prev_box.action == 'hs'){
          ...
          next = prev_box.cases.find(i => i.content[1] == this.activeCases[prev_box.id] ).next
        }
        let arr = (action=='ad') ? [val, val] : val ;
        this.boxes.push({
            id: this.id,
        ...
        if(prev_box.action == 'hs'){
          ...
          let ad_case = prev_box.cases.find(i => i.content[0] == this.activeCases[prev_box.id][0] && i.content[1] == this.activeCases[prev_box.id] )
            if(ad_case){
                ad_case.next = this.id
            }else{
              this.setPrevBox(prev_box.id);
              // prev_box.cases.push({
